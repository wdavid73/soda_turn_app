...
class DayEditSheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final day = data.assignments[dateIso] ?? const DayAssignmentEntity();
    final hasRecord = data.assignments.containsKey(dateIso);
    final present = hasRecord ? day.present : data.activeIds;
    final mondayIso = AppDateUtils.toIso(
      AppDateUtils.mondayOf(AppDateUtils.parseIso(dateIso)),
    );
    final vasosId = data.weeklyVasos[mondayIso]?.personId;
            ...
            children: [
              Row(
                children: [
                  ...
                  IconButton(
                    onPressed: () => vm.toggleDayLock(dateIso),
                ...
                children: [
                  for (final id in present)
                    ChoiceChip(
                      ...
                      selected: day.gaseosa == id,
                      selectedColor: AppTheme.mint,
                      onSelected: (selected) =>
                          vm.setGaseosa(dateIso, selected ? id : null),
                    ),
                ],
              ),
              ...
              Row(
                children: [
                  ...
                  if (day.gaseosa != null)
                    TextButton(
                      onPressed: () => vm.setGaseosa(dateIso, null),
